...
int deleteElement(BTDescriptor* dsTree, BTNode* subTree, char* key, int data);
...
int main()
{
    ...
    int dataToDelete = 0; //для цілого числа вершини, яку потрібно вилучити з дерева
...
його*/
    ...
    int deleteCheck = 0; //для перевірки на помилки при видаленні елемента
...
типу*/
        ...
        case 3: //виключення елемента з дерева
        {
            ...
            if (searchForData(BTd->root, forKey, &dataToDelete) == 0) /*пошук елементу в дереві за ключем*/
            {
                printf("Елементу з таким ключем немає в дереві!\n");
            }
            deleteCheck = deleteElement(BTd, BTd->root, forKey, dataToDelete);
            //вилучення елементу з дерева
            if (deleteCheck == -1) /*вершина, яку бажає вилучити користувач, відсутня в дереві*/
            {
                printf("Вершина, яку Ви бажаєте видалити, у дереві відсутня!\n");
                free(forKey); //звільнення пам'яті для введеного з клавіатури ключа
                break;
            }
            else if (deleteCheck == -2) /*користувач передач адресу неіснуючого
дескриптора дерева*/
            {
                printf("Ви передали адресу неіснуючого дескриптора дерева!\n");
                free(forKey); //звільнення пам'яті для введеного з клавіатури ключа
                exit(0);
            }
            else if (deleteCheck == -3) /*користувач передав дескриптор виродженого дерева*/
            {
                printf("Ви передали дескриптор виродженого дерева!\n");
                free(forKey); //звільнення пам'яті для введеного з клавіатури ключа
                break;
            }
            counter--;
...
Вхід:
    ...
    data - змінна з цілим числом шуканого елемента
Вихід:
    deleteElement = 0 - якщо елемент видалено
                  = -1 - якщо елемент не видалено
                  = -2 - якщо переданий неіснуючий дескриптор дерева
                  = -3 - якщо дерево вироджене
*/
int deleteElement(BTDescriptor* dsTree, BTNode* subTree, char* key, int data) {
    ...
    if (dsTree == NULL)
    {
        return -2; //переданий неіснуючий дескриптор дерева
    }
    ...
    while (current != NULL && strcmp(key, current->treeData.key) != 0) {
        parent = current;
        current = (strcmp(key, current->treeData.key) > 0) ? current->left :
            current->right;
    }
    if (!current)
    {
        return -1; //вершина, що видаляється, в дереві відсутня
    }
    /********************Вершина, що видаляється, знайдена********************/

    //Випадок 1, присутність двох нащадків у вершини, що видаляється
    if (current->left && current->right) {
        /*пошук мінімального елемента у правому піддереві з переносом його на місце
        видаленої вершини*/
        current->treeData.data = findSmallestElement(current->right)->treeData.data;
        current->treeData.key = findSmallestElement(current->right)->treeData.key;
        //видалення мінімального елемента у правому піддереві
        deleteElement(dsTree, current->right, current->treeData.key,
            current->treeData.data);
        return 0;
    }
    //Випадок 2, у вершини, що видаляється, відсутні нащадки
    else if (current->left == NULL && current->right == NULL) {

        //якщо вершина, що видаляється стоїть зліва від батьківської
        if (parent->left == current)
        {
            free(parent->left->treeData.key); /*звільнення пам'яті під поле key лівого
нащадка*/
            free(current); //звільнення пам'яті під лівий нащадок
            parent->left = NULL;
        }
        //якщо справа від батьківської
        else
        {
            free(parent->right->treeData.key); /*звільнення пам'яті під поле key
правого нащадка*/
            free(current); //звільнення пам'яті під правий нащадок
            parent->right = NULL;
        }
    }

    //Випадок 3, один нащадок у вершини, що видаляється
    else if (current->left != NULL) //якщо нащадок лівий
    {
        parent = current;
        current = current->left;
        strcpy(parent->treeData.key, current->treeData.key);
        parent->treeData.data = current->treeData.data;
        free(current); //звільнення пам'яті під лівий нащадок
        parent->left = NULL;
    }
...
для перепису ключа з файлу до нього*/
        ...
        for (int j = strlen(data) - 1, g = 0; j >= 0; j--, g++)
        {
            ...
            {
                dAK[i].data = dAK[i].data + (data[j] - 48) * pow(10, g);
            }
        }
    }
    rewind(fp);
